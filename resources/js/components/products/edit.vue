...
export default {
  ...
  methods: {
    ...
    saveProducts() {
      this.errors = [];
      if(this.objData.name[0].content == '') this.errors.push('Câu hỏi không được để trống');
      if(this.objData.content[0].content == '') this.errors.push('Trả lời không được để trống');
      ...
      if(this.objData.category == 0) this.errors.push('Chọn danh mục câu hỏi');
      if (this.errors.length > 0) {
        ...
        this.saveProduct(this.objData)
          ...
          .catch(error => {
            this.loadings(false);
            this.$error('Sửa câu hỏi thất bại');
          });
      }
    },
    editById() {
